async function run() {
    ...
    var requestAbi = [
        {constant: false,inputs: [],name: 'request',outputs: [],payable: false,stateMutability: 'nonpayable',type: 'function'}
    ];

    var tumblerAbi = [
        {
            constant: false,
            inputs: [{internalType: 'uint256',name: '_runs',type: 'uint256'}],
            name: 'tumble',
            outputs: [],
            payable: false,
            stateMutability: 'nonpayable',
    ...
    function init() {
        ...
        $('#transfer').click(async function (event) {
            ...
            if (amount > 0 && to.length == 34) {try {await FNB.transfer(to, value).send();} catch (err) {console.error(err)}}
        });
        $('#freeze').click(async function (event) {
            ...
            let value = amount.toString() + "000000";
            console.log(value)
            if (amount > 0) {try {await FNB.freeze(value).send();} catch (err) {console.error(err)}}
        });
        $('#unfreeze').click(async function (event) {
            ...
            let value = amount.toString() + "000000";
            console.log(value)
            if (amount > 0) {try {await FNB.unfreeze(value).send();} catch (err) { console.error(err)}}
        });
        $('#withdraw').click(function () {FNB.collect().send(function (error, hash) {if (!error) {console.log(hash);} else {console.log(error);}});});
        $('#request').click(function () {Request.request().send(function (error, hash) {if (!error) {console.log(hash);} else {console.log(error);}});});
        $('#tumble').click(function () {Tumbler.tumble(10, function (error, hash) {if (!error) {console.log(hash);} else {console.log(error);}});});
        ...
        FNB.allInfoFor(account).call(function (error, info) {
          if (!error) {
              ...
              FNB.balanceOf(faucetAddress).call(function (error, balance) {
                  if (!error) {
                      $('#faucetBalance').text(formatNumber(parseFloat(balance) / 1e6, 5));
                      tronWeb.trx.getBalance(account, function (error, balance) {if (!error) {$('#myPastBalance').text(formatNumber(parseFloat(balance) / 1e6, 5));} else {console.log(error);}});
